...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<DeviceException> onRegularException(DeviceException e, HandlerMethod handlerMethod, Locale locale) {
        String message = handleException(handlerMethod, locale);
        LOGGER.log(Level.SEVERE, message, e);
        StringWriter stringWriter = new StringWriter();
        e.printStackTrace(new PrintWriter(stringWriter));

        return new ResponseEntity<>(new DeviceException(String.format("%s%s", message, stringWriter.toString()),
